...
$(function () {
  ...
  function renderDefaults(q, sync) {
    if (q === '') {
      sync(prefetchExample.get('São Pedro', 'Campo Limpo Paulista', 'São Paulo'));
    } else {
  ...
  $('.typeahead-custom-template').typeahead(null, {
    ...
    templates: {
      empty: [
        '<div class="empty-message p-2">',
        'Cidade não encontrada! Digite corretamente...',
        '</div>'
      ].join('\n'),
      suggestion: function (data) {
        return '<div><span class="fw-medium">' + data + '</span> – </div>';
      }
    }
  });
